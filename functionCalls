...
for (i in 1:dim(x)[2])
{
  ...
  print(special.Vars)
  overallDiagsPlots(xmat = x, yvar = y, noiseLevels = noiseLevs, spec.Vars = special.Vars, iter = iteration, choice =  c("d"))
}
...
boxplotoutperc = BoxplotAllPerc(xmatrix = x, y = y, noiseLevs = noiseLevs, special.Vars = special.Vars, iteration = iteration)
